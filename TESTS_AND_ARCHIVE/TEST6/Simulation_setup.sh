...
if [ $ELECTROSTATIC = "True" ]; then
	MOF_PWD="$BASE_DIR/BUILD/resources/oRE-MOF-1.0-DFT-Minimized/minimized_structures_with_DDEC_charges/${MOF_FILE}"
elif [ $ELECTROSTATIC = "False" ]; then 
...
AX=$( echo "$CELL_LENGTH_A*$SUPERCELL_X" | bc -l )
AY='0'
AZ='0'
BX=$( echo "$CELL_LENGTH_B*c($GAMMA)*$SUPERCELL_Y" | bc -l )
BY=$( echo "$CELL_LENGTH_B*s($GAMMA)*$SUPERCELL_Y" | bc -l )
BZ='0'
CX=$( echo "$CELL_LENGTH_C*c($BETA)*$SUPERCELL_Z" | bc -l )
CY=$( echo "$CELL_LENGTH_C*c($ALPHA)*s($GAMMA)*$SUPERCELL_Z" | bc -l )
CZ=$( echo "$CELL_LENGTH_C*s($BETA)*$SUPERCELL_Z" | bc -l )
...
#########################
cd ../reservoir_base
cp "../../../BUILD/resurces/pdb/${ADSORBATE_FILE}" .
