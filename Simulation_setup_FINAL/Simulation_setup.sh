...
cp ../../../BUILD/resources/pack/replacement_names.dat .
...
if [ $ELECTROSTATIC = "True" ]; then
	cat $MOF_FILE | grep -Pzo '.*atom_site_charge(.*\n)*' | sed '1d' > cut_cif.dat
	...
	echo "building PDB and PSF file for MOF:Start"
	...
	echo "electrostatic not defined"
	exit
fi
...
for F in "${uniqatoms[@]}" ; do
	echo "Fugacity $F 0" >> fugacity.dat
done
sed -i -e '/Fugacity/r fugacity.dat' in.conf
